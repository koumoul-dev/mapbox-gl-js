 ...
 * ```
 *
 * If we let `ratio = (y1 - y0) * (base^(x-x0) - 1) / (base^(x1-x0) - 1) `,
 * then we have `f(x) = y0 + (y1 - y0) * ratio`.  In other words, `ratio`
 * may be treated as an interpolation factor between the two stops' output
 * values.
 *
 * (Note: a slightly different form for `ratio`,
 * `(base^x - base^x0) / (base^x1 base^x0)`, is equivalent, but requires fewer
